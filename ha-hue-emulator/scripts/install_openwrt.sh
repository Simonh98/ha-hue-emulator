...
cp -r HueObjects configManager flaskUI functions lights logManager sensors services /opt/ha-hue-emulator/
...
wait
mv index.html /opt/ha-hue-emulator/flaskUI/templates/
cp -r static /opt/ha-hue-emulator/flaskUI/

echo -e "\033[32m Copying custom network function for openwrt.\033[0m"
rm -Rf /opt/ha-hue-emulator/BridgeEmulator/functions/network.py
mv /opt/tmp/diyHue/BridgeEmulator/functions/network_OpenWrt.py /opt/ha-hue-emulator/functions/network.py
...
cd /opt/ha-hue-emulator
mkdir -p /opt/ha-hue-emulator/config
...
wait
touch /opt/ha-hue-emulator/config/cert.pem
cat private.key > /opt/ha-hue-emulator/config/cert.pem
cat public.crt >> /opt/ha-hue-emulator/config/cert.pem
...
chmod +x /opt/ha-hue-emulator/HueObjects
chmod +x /opt/ha-hue-emulator/configManager
chmod +x /opt/ha-hue-emulator/flaskUI
chmod +x /opt/ha-hue-emulator/functions
chmod +x /opt/ha-hue-emulator/lights
chmod +x /opt/ha-hue-emulator/logManager
chmod +x /opt/ha-hue-emulator/sensors
chmod +x /opt/ha-hue-emulator/services
