...
generate_certificate () {
  ...
  if [ $? -ne 0 ] ; then
    ...
    curl "https://certgen.lightningdark.com/gencert?mac=$mac" > /opt/ha-hue-emulator/cert.pem
  else
    touch /opt/ha-hue-emulator/cert.pem
    cat private.key > /opt/ha-hue-emulator/cert.pem
    cat public.crt >> /opt/ha-hue-emulator/cert.pem
    rm private.key public.crt
  fi

  if [ ! -d "/opt/ha-hue-emulator/config/" ]; then
    mkdir /opt/ha-hue-emulator/config/ -p
  fi
  cp /opt/ha-hue-emulator/cert.pem /opt/ha-hue-emulator/config/
...
cp -r HueEmulator3.py HueObjects configManager flaskUI functions lights logManager sensors services /opt/ha-hue-emulator/
