...
cp default-config.json /opt/ha-hue-emulator/config.json
cp default-config.json /opt/ha-hue-emulator/default-config.json
cp -r HueObjects configManager flaskUI functions lights /opt/ha-hue-emulator/
cp -r logManager sensors services /opt/ha-hue-emulator/
...
curl "http://mariusmotea.go.ro:9002/gencert?mac=$mac" > /opt/ha-hue-emulator/cert.pem
