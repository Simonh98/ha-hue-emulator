...
class Bridge:
    ...
    def __init__(self, config_service: Config) -> None:
        ...
        mac = config_service.mac.replace(':', '')
        self.bridgeid = f"{mac[:6]}FFFE{mac[-6:]}".upper()
        self.modelid = "BSB002"
    ...
    def add_light(self, light: Light):
        lightcopy = copy.deepcopy(light)
        with self.mutex:
            self.lights.append(lightcopy)
        log.info(f"Added light -> ({lightcopy.name})")
